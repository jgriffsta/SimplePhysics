...
function draw() { 
	...
	for(var i = 0;i<ballObject.length;i++){
		if(ballObject[i].visible)drawCircle(ballObject[i].posX*mpp,(canvasH-ballObject[i].posY*mpp),ballObject[i].radius*mpp);
...
function physics(){
	...
	for (var i = 0;i<ballObject.length;i++){
		...
		if(ballObject[i].posY-ballObject[i].radius<0)ballObject[i].posY= ballObject[i].radius;
...
function collision(i,y){
	...
	var k1 = ballA.velX*Math.sin(alpha)-ballA.velY*Math.sin(theta);
	var k2 = ballB.velX*Math.sin(alpha)-ballB.velY*Math.sin(theta);
